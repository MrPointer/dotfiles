...
A Zsh shell session starts after the [chezmoi data contract][chezmoi-data-contract] rendered the machine with `personal.work_env` set to `true`. This process is a sub-flow of [shell startup][shell-startup].
...
- [Chezmoi data contract][chezmoi-data-contract]: `personal.work_env`, `system.work_generic_dotfiles_profile`, and `system.work_specific_dotfiles_profile` must be set for a work environment
