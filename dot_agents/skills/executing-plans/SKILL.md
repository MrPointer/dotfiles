...
**Isolation via isolated workspace**: The test author must NOT have access to plan files. Use the active runtime adapter's isolation mechanism to provide an isolated workspace containing the relevant code surface plus any explicitly seeded ignored build/cache artifacts, but not the planning artifacts.
